...
# env_ctm.update_(opts_svd=opts_svd_ctm, method="2site")
...
for step in tqdm(range(n_steps)):
    # env_ctm.psi = psi
    # env_ctm.reset_(init="eye")

    # env_ctm.update_(opts_svd=opts_svd_ctm, method="2site")

    # ctm_out_before = env_ctm.ctmrg_(
    #     opts_svd=opts_svd_ctm,
    #     max_sweeps=50,      # can tune; more sweeps => better env, more cost
    #     # method="1site",   # <= IMPORTANT for tiny finite lattices like 2x2
    #     # corner_tol=1e-8   # you can add convergence criterion if desired
    # )
    ...
    infos = fpeps.evolution_step_(
        ...
        method='mpo',       # default; 'NN' splits into NN gates if using longer-range MPO gates
...
Delta_trunc = fpeps.accumulated_truncation_error(infoss, statistics='mean')


print("----------------------------------------------------------------------------")
print(f"Ground State Energy per Site using fPEPS (full update): {energy}") # type: ignore
print(f"Exact Ground State Energy per Site (ED):               {ed_energy}")
print(f"Relative Energy Error: {abs((energy - ed_energy) * 100 / ed_energy): .5f} %") # type: ignore
print("----------------------------------------------------------------------------")
...
print("----------------------------------------------------------------------------")
print(f"Accumulated truncation error Δ (mean over bonds): {Delta_trunc}")
print("----------------------------------------------------------------------------")
